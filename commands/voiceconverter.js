...
let convertToWav = file => {
    ...
    AouraQueen['IntroduceCMD']({ pattern: 'voicy', desc: DATA.USAGE, fromMe: WorkType }, (async (message, input) => {
        try {
            if (message.reply_message) {
                if (!message.reply_message.text && !message.reply_message.video && !message.reply_message.image) {
                    ...
                    convertToWav(file).on('end', async () => {
                        ...
                        await message.client.sendMessage(message.jid, DATA.TEXT + ''```' + recognizedText + *👑ᴘᴏᴡᴇʀᴅ ʙʏ λᴏᴜᴇʀɑ ǫᴜᴇᴇɴ👑*\n\n'```'', MessageType.text)
                    });
                } else {
