...
AouraQueen['IntroduceCMD']({pattern: 'speedtest ?(.*)', fromMe: true, desc: DATA.SPEEDTEST_DESC, usage: 'speedtest user // speedtest server'}, (async (message, input) => {
    if (input[1] == 'user' || input[1] == 'User') {
        ...
        var payload = auth_message.download + realspeed_msg + '\n' + 
            ...
            auth_message.gb_speed_d + user_download.gbps + '*👑ᴘᴏᴡᴇʀᴅ ʙʏ λᴏᴜᴇʀɑ ǫᴜᴇᴇɴ👑*\n\n'
...
AouraQueen['IntroduceCMD']({pattern: 'ping$', fromMe: true, delownsewcmd: false, desc: DATA.PING_DESC}, (async (message, input) => {
  var start = new Date().getTime();
  ...
  await message.client.sendMessage(
    ...
    AouraQueen['IntroduceCMD']({pattern: 'short ?(.*)', fromMe: WorkType, desc: DATA.URL}, (async (message, input) => {
    ...
    AouraQueen['IntroduceCMD']({pattern: 'cal ?(.*)', fromMe: WorkType, desc: 'Powerfull Calculater With >  - + * / × ÷ π e ( ) pie pi log lg ^ 1-9'}, (async (message, input) => {
       var textsew = '';
